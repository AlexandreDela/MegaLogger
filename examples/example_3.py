...
class Solar_Sensor(megalogger.MegaLoggedItemString):
    def __init__(self,
                 power,
                 unique_id,
                 has_backsheet: bool):
    ...
    def to_string(self,
                 **kwargs) -> str:
        return (f"<{self.power}, {self.life_expectancy}, "
                f"{self.unique_id}, {self.has_backsheet}>")


class MyPandasLogger(megalogger.DelayedStreamLoggerBlueprint):
    def add_item(self,
                 item,
                 message,
                 **kwargs):
        if isinstance(item, Solar_Sensor):
            tuple_equivalent = item.to_string() + "," + message
            self.internal_list_string.append(tuple_equivalent)
        else:
            raise TypeError()
